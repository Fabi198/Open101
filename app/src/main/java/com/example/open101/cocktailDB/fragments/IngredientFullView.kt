...
class IngredientFullView : Fragment(R.layout.fragment_ingredient_full_view) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val ingredient = arguments?.getString("Ingredient")?.replace(" ", "_")
        ...
        if (ingredient != null) {
            ...
            if (id > 0) {
                lifecycleScope.launch(Dispatchers.IO) {
                    val translator = TranslateService
                    val responseService = RetrofitCocktail.APICOCKTAILS.getIngredient("lookup.php?iid=$id")
                    val ingredientDTO = responseService.body()
                    val responseService2 = RetrofitCocktail.APICOCKTAILS.getPopularDrinks("filter.php?i=${ingredientDTO?.ingredient?.get(0)?.strIngredient?.replace(" ", "_")}")
                    val cocktails = responseService2.body()
                    withContext(Dispatchers.Main) {
                        if (responseService.isSuccessful) {
                            Picasso.get().load("https://www.thecocktaildb.com/images/ingredients/${ingredientDTO?.ingredient?.get(0)?.strIngredient?.replace(" ", "%20")}-Small.png").fit().into(binding.ivIngredientBackground)
                            binding.tvIngredientTitle.text = ingredientDTO?.ingredient?.get(0)?.strIngredient
                            binding.tvType.text = "Type:\n${ingredientDTO?.ingredient?.get(0)?.strType}"
                            binding.tvABV.text = "ABV:\n${ingredientDTO?.ingredient?.get(0)?.strABV}"
                            binding.tvAlcoholic.text = "Alcoholic:\n${ingredientDTO?.ingredient?.get(0)?.strAlcohol}"
